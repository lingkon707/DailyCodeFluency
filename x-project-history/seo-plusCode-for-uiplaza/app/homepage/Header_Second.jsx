...
export default function Header() {
  ...
  return (
    <header className="bg-gradient-to-r from-white via-blue-50 to-white shadow-md sticky top-0 z-50 font-sans">
      <div className="container mx-auto flex justify-between items-center py-4 px-6">
        {/* Logo */}
        <div className="flex flex-col">
          <h1 className="text-2xl font-extrabold text-blue-600">UIPlaza</h1>
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-slate-700 hover:text-blue-600 transition font-medium text-sm"
            >
              {link.name}
            </a>
          ))}
          ...
          <div className="relative">
            <input
              ...
              className="bg-white border border-slate-300 text-slate-700 rounded-full pl-10 pr-4 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
            ...
            <div className="flex flex-col space-y-6 text-slate-800">
              ...
              <div className="relative mt-4">
                <input
                  type="text"
                  placeholder="Search UI assets..."
                  className="bg-white border border-slate-300 rounded-full pl-10 pr-4 py-2 text-slate-700 w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
                <MagnifyingGlassIcon className="h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400" />
              </div>
